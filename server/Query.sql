...
--     group by
--       p.id, w.id
--       order by p.id, p."dateInit";
--   -- ) as g on g.works_id = wo.id
--   -- join users us on wo.users_id = us.id
--   -- join credentials ce on ce.users_id = us.id;
-- WITH recursive date_series AS (
--   SELECT 
--     start_date AS date
--   FROM 
--     (SELECT DATE '2023-07-10' AS start_date, DATE '2023-07-15' AS end_date) AS initial_range
--   WHERE 
--     EXTRACT(dow FROM start_date) IN (1, 2) -- Filter the desired weekdays (1: Monday, 2: Tuesday)
...
--     date_series
--   WHERE 
--     date + INTERVAL '1' DAY <= (SELECT end_date FROM initial_range)
-- )
-- SELECT 
--   date
-- FROM 
